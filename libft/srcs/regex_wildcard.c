...
static int	regex_mark(t_regex_info *rgxi, int *len)
{
	int	neg;
...
int		regex_wildcard(t_regex_info *rgxi, int *len)
{
	int			ret;
	int			neg;
	const char	*o_str;
	...
	if (!(neg = 0) && *rgxi->regex == '*')
	{
		...
		ret = 0;
		while (*o_str && (ret = regex_exec(rgxi)) == -1 && ++*len)
		{
			rgxi->str = ++o_str;
			rgxi->regex = o_rgx;
		}
